...
public interface ResearchGapRepository extends JpaRepository<ResearchGap, UUID> {
         ...
         */
        @Query("SELECT rg FROM ResearchGap rg WHERE rg.validationConfidence >= :minConfidence ORDER BY rg.validationConfidence DESC")
         ...
         */
        @Query("SELECT rg FROM ResearchGap rg WHERE rg.gapAnalysis.paper.id = :paperId AND rg.validationStatus = 'VALID' ORDER BY rg.validationConfidence DESC")
         ...
         */
        void deleteByGapAnalysisIdIn(List<UUID> gapAnalysisIds);
}
