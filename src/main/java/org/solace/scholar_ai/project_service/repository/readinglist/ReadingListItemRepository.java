...
public interface ReadingListItemRepository extends JpaRepository<ReadingListItem, UUID> {
        ...
        // Tag-based search
        @Query(value = "SELECT * FROM reading_list WHERE project_id = :projectId AND :tag = ANY(tags) ORDER BY added_at DESC", nativeQuery = true)
        List<ReadingListItem> findByProjectIdAndTag(@Param("projectId") UUID projectId, @Param("tag") String tag);

        // Full-text search
        @Query(value = "SELECT * FROM reading_list WHERE project_id = :projectId AND (LOWER(notes) LIKE LOWER(CONCAT('%', :searchTerm, '%'))) ORDER BY added_at DESC", nativeQuery = true)
        ...
        @Query(value = "SELECT * FROM reading_list WHERE project_id = :projectId "
                        ...
                        + "ORDER BY added_at DESC", nativeQuery = true)
        ...
        long countByProjectIdAndStatus(@Param("projectId") UUID projectId,
                        @Param("status") ReadingListItem.Status status);
        ...
        @Query("SELECT SUM(r.estimatedTime) FROM ReadingListItem r WHERE r.projectId = :projectId AND r.estimatedTime IS NOT NULL")
        Long getTotalEstimatedTimeByProjectId(@Param("projectId") UUID projectId);

        @Query("SELECT SUM(r.actualTime) FROM ReadingListItem r WHERE r.projectId = :projectId AND r.actualTime IS NOT NULL")
        ...
        @Query("SELECT r FROM ReadingListItem r WHERE r.projectId = :projectId AND r.isRecommended = true ORDER BY r.addedAt DESC")
