...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, UUID> {
         ...
         */
        @Query("SELECT cm FROM ChatMessage cm WHERE cm.sessionId = :sessionId AND cm.tokenCount IS NOT NULL ORDER BY cm.timestamp DESC")
        List<ChatMessage> findMessagesWithTokenCount(@Param("sessionId") UUID sessionId);

        /**
         * Delete chat messages by chat session IDs
         */
        void deleteByChatSessionIdIn(List<UUID> chatSessionIds);
}
