...
public interface LatexDocumentCheckpointRepository extends JpaRepository<LatexDocumentCheckpoint, Long> {
         ...
         */
        List<LatexDocumentCheckpoint> findByDocumentIdAndSession_IdOrderByCreatedAtDesc(UUID documentId,
                        UUID sessionId);
         ...
         */
        @Modifying
        @Query(value = "DELETE FROM latex_document_checkpoints c WHERE c.document_id = :documentId AND c.id NOT IN (SELECT id FROM latex_document_checkpoints WHERE document_id = :documentId ORDER BY created_at DESC LIMIT :keepCount)", nativeQuery = true)
