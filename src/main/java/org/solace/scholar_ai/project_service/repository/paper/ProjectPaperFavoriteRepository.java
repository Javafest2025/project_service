...
public interface ProjectPaperFavoriteRepository extends JpaRepository<ProjectPaperFavorite, UUID> {
        ...
        @Query("SELECT f FROM ProjectPaperFavorite f JOIN f.paper p WHERE f.project.id = :projectId AND p.title LIKE %:title%")
        List<ProjectPaperFavorite> findByProjectIdAndPaperTitleContaining(@Param("projectId") UUID projectId,
                        @Param("title") String title);

        // Search favorites by paper title for a user
        @Query("SELECT f FROM ProjectPaperFavorite f JOIN f.paper p WHERE f.project.id = :projectId AND f.userId = :userId AND p.title LIKE %:title%")
        List<ProjectPaperFavorite> findByProjectIdAndUserIdAndPaperTitleContaining(@Param("projectId") UUID projectId,
