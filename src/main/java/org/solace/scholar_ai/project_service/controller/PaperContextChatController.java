...
public class PaperContextChatController {
    ...
    public ResponseEntity<?> getChatSessionHistory(
            @Parameter(description = "ID of the paper", required = true) @PathVariable String paperId,
            @Parameter(description = "ID of the chat session", required = true) @PathVariable String sessionId) {
        ...
        try {
            ...
            log.error(
                    ...
                    e.getMessage(),
                    e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error retrieving chat history");
        }
    }
}
