...
public class CitationAnalysisService {
    ...
    private static final Pattern CLAIM_PATTERN = Pattern.compile("\\b(shows?|demonstrates?|proves?|indicates?|suggests?|confirms?|reveals?|establishes?)\\b");
    
    /**
     * Analyze LaTeX content for citation issues using selected papers as local context
     */
    public List<CitationIssue> analyzeDocument(CitationCheck check, String latexContent, List<String> selectedPaperIds, boolean runWebCheck) {
        log.info("Starting citation analysis for document {} with {} selected papers", check.getDocumentId(), selectedPaperIds.size());
        ...
        try {
            ...
            log.info("Loaded local corpus: {} papers, {} paragraphs, {} references", 
                localCorpus.size(), localCorpus.values().stream().mapToInt(List::size).sum(),
    ...
    private List<LatexSentence> parseLatexIntoSentences(String latexContent) {
        ...
        for (int lineNumber = 0; lineNumber < originalLines.length; lineNumber++) {
            ...
            for (int i = 0; i < sentenceParts.length; i++) {
                ...
                if (sentenceText.length() > 10) { // Minimum sentence length
                    ...
                    log.debug("Created sentence at line {}: '{}' with cited keys: {}", lineNumber + 1, sentenceText.substring(0, Math.min(50, sentenceText.length())), citedKeys);
                }
        ...
        log.info("Parsed {} sentences from LaTeX content with line numbers ranging from {} to {}", 
            sentences.size(), 
            sentences.isEmpty() ? 0 : sentences.stream().mapToInt(LatexSentence::getLineStart).min().orElse(0),
            sentences.isEmpty() ? 0 : sentences.stream().mapToInt(LatexSentence::getLineStart).max().orElse(0));
    ...
    private Set<String> extractCitedKeys(String text) {
        ...
        while (matcher.find()) {
            ...
            Arrays.stream(citeContent.split(","))
                .map(String::trim)
                .forEach(keys::add);
        }
        return keys;
     ...
     */
    private List<CitationIssue> analyzeSentence(CitationCheck check, LatexSentence sentence, 
        ...
        try {
            ...
            if (needsCitation && !hasAdequateCitation) {
                CitationIssue.IssueType issueType = sentence.getCitedKeys().isEmpty() ? 
                    CitationIssue.IssueType.MISSING_CITATION : CitationIssue.IssueType.WEAK_CITATION;
                ...
                log.info("Created citation issue at line {}: '{}'", 
                    sentence.getLineStart(), 
                    sentence.getText().substring(0, Math.min(50, sentence.getText().length())));
     ...
     */
    private LocalVerificationResult performLocalVerification(LatexSentence sentence, 
            Map<String, List<ExtractedParagraph>> localCorpus) {
        ...
        List<EvidenceCandidate> topCandidates = candidates.stream()
            .limit(5)
            .collect(Collectors.toList());
        ...
        for (EvidenceCandidate candidate : topCandidates) {
            VerificationDecision decision = verifyWithAI(sentence.getText(), candidate.getParagraph().getText());
    ...
    private boolean needsCitation(LatexSentence sentence, LocalVerificationResult localResult) {
        ...
        return !localResult.getVerifiedEvidence().isEmpty() && sentence.getCitedKeys().isEmpty();
    }
    ...
    private List<Map<String, Object>> createSuggestions(LocalVerificationResult localResult) {
        ...
        for (VerifiedEvidence evidence : localResult.getVerifiedEvidence()) {
            try {
                ...
                Optional<Paper> paperOpt = paperRepository.findById(UUID.fromString(evidence.getCandidate().getPaperId()));
                ...
                log.warn("Failed to create suggestion for paper {}: {}", evidence.getCandidate().getPaperId(), e.getMessage());
    ...
    public static class LatexSentence {
        ...
        public Set<String> getCitedKeys() { return citedKeys; }
    }
    ...
    public static class EvidenceCandidate {
        ...
        public ExtractedParagraph getParagraph() { return paragraph; }
    ...
    public static class VerifiedEvidence {
        ...
        // Getters
        public EvidenceCandidate getCandidate() { return candidate; }
    ...
    public static class LocalVerificationResult {
        ...
        // Getters
        public List<EvidenceCandidate> getCandidates() { return candidates; }
        public List<VerifiedEvidence> getVerifiedEvidence() { return verifiedEvidence; }
    }
    
    /**
     * Find orphan references - bibliography entries that are never cited in the text
     */
    private List<CitationIssue> findOrphanReferences(CitationCheck check, String latexContent, List<LatexSentence> sentences) {
        List<CitationIssue> issues = new ArrayList<>();
        ...
        try {
            ...
            Set<String> citedKeys = sentences.stream()
                .flatMap(s -> s.getCitedKeys().stream())
                .collect(Collectors.toSet());
     ...
     */
    private List<CitationIssue> findDanglingCitations(CitationCheck check, String latexContent, List<LatexSentence> sentences) {
        List<CitationIssue> issues = new ArrayList<>();
     ...
     */
    private List<CitationIssue> validateMetadata(CitationCheck check, String latexContent, Map<String, List<ExtractedReference>> localReferences) {
        List<CitationIssue> issues = new ArrayList<>();
                        ...
                        if (isLikelyMatch(bibEntry, ref)) {
                            ...
                            for (String metadataIssue : metadataIssues) {
                                ...
                                CitationIssue issue = CitationIssue.builder()
                                    ...
                                    .projectId(check.getProjectId())
                                    .documentId(check.getDocumentId())
                                    .type(CitationIssue.IssueType.WEAK_CITATION) // Could add INCORRECT_METADATA type
                                    .severity(CitationIssue.Severity.MEDIUM)
                                    .fromPos(bibPosition)
                                    .toPos(bibPosition + bibKey.length())
                                    .lineStart(lineNumber)
                                    .lineEnd(lineNumber)
                                    .snippet("Metadata issue in bibliography entry '" + bibKey + "': " + metadataIssue)
                                    .citedKeys(new String[]{bibKey})
     ...
     */
    private List<CitationIssue> detectPotentialPlagiarism(CitationCheck check, List<LatexSentence> sentences, Map<String, List<ExtractedParagraph>> localCorpus) {
        List<CitationIssue> issues = new ArrayList<>();
                        ...
                        if (paragraph.getText() != null && paragraph.getText().length() > 30) {
                            ...
                            if (similarity > 0.85 && sentence.getCitedKeys().isEmpty()) {
                                CitationIssue issue = CitationIssue.builder()
                                    ...
                                    .projectId(check.getProjectId())
                                    .documentId(check.getDocumentId())
                                    .type(CitationIssue.IssueType.MISSING_CITATION) // Could add POSSIBLE_PLAGIARISM type
                                    .severity(CitationIssue.Severity.HIGH)
                                    .fromPos(sentence.getStartPos())
                                    .toPos(sentence.getEndPos())
                                    .lineStart(sentence.getLineStart())
                                    .lineEnd(sentence.getLineEnd())
                                    .snippet("High similarity to source material without citation (similarity: " + String.format("%.2f", similarity) + ")")
                                    .citedKeys(new String[]{})
                                ...
                                issues.add(issue);
                                log.debug("Found potential plagiarism: similarity {} with paper {}", similarity, paperId);
    ...
    private Map<String, BibliographyEntry> parseBibliographyEntries(String latexContent) {
        ...
        Pattern bibitemPattern = Pattern.compile("\\\\bibitem\\{([^}]+)\\}\\s*([^\\\\]*?)(?=\\\\bibitem|\\\\end\\{thebibliography\\}|$)", Pattern.DOTALL);
        Matcher matcher = bibitemPattern.matcher(latexContent);
    ...
    private List<String> validateBibliographyMetadata(BibliographyEntry bibEntry, ExtractedReference ref) {
        ...
        if (bibYear != null && ref.getYear() != null && !bibYear.equals(ref.getYear().toString())) {
    ...
    private String generateBibTexEntry(Paper paper) {
        try {
            ...
            bibtex.append("  author={").append(escapeBibTeX(getAuthorsString(paper))).append("},\n");
            
            if (paper.getPublicationDate() != null) {
                bibtex.append("  year={").append(paper.getPublicationDate().getYear()).append("},\n");
            }
